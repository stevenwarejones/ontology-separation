...
Formalized reproduction of Baumann & Brukner, Quantum 8, 1481 (2024),
Sec. 3. No novelty claim or human domain-expert review is implied.
  ...
  trace_rankOne_eq_one := by
    apply Complex.ext <;>
      norm_num [QIT.rankOneMatrix, Matrix.vecMulVec, Matrix.trace,
        Fintype.sum_prod_type, Fintype.sum_bool, map_ofNat, s] <;>
...
def computational : QIT.ProjectiveMeasurement Bool Bool where
  ...
  orthogonal := by
    ...
    cases i <;> cases j <;> cases r <;> cases c <;>
      norm_num [computationalEffect, Matrix.mul_apply, Fintype.sum_bool] at *
  sum_eq_one := by
    ext i j
    cases i <;> cases j <;>
...
def rotated : QIT.ProjectiveMeasurement Bool Bool where
  ...
  orthogonal := by
    ...
    apply Complex.ext <;>
      ...
      norm_num [rotatedEffect, Matrix.mul_apply, Fintype.sum_bool, r,
        BaumannBruknerProtocol.sqrtTwo] at * <;>
      nlinarith [Real.sq_sqrt (show (0 : ℝ) ≤ 2 by norm_num)]
  sum_eq_one := by
    ext i j
    cases i <;> cases j <;>
      norm_num [rotatedEffect, Matrix.one_apply, Fintype.sum_bool] <;> ring
...
theorem computational_conditional (b f g : Bool) :
  ...
  cases b <;> cases f <;> cases g <;>
    ...
    norm_num [conditionalFriend, computational, computationalEffect,
      ...
      Matrix.vecMulVec, Fintype.sum_bool, map_ofNat, s] <;>
    nlinarith [Real.sq_sqrt (show (0 : ℝ) ≤ 2 by norm_num)]
...
theorem rotated_conditional (b f g : Bool) :
  ...
  cases b <;> cases f <;> cases g <;>
    ...
    norm_num [conditionalFriend, rotated, rotatedEffect,
      state, source, QIT.PureVector.state, QIT.rankOneMatrix,
      Matrix.vecMulVec, Fintype.sum_bool, map_ofNat, s, r,
      BaumannBruknerProtocol.sqrtTwo] <;>
    nlinarith [Real.sq_sqrt (show (0 : ℝ) ≤ 2 by norm_num),
      congrArg (fun x : ℝ => Real.sqrt 2 * x)
