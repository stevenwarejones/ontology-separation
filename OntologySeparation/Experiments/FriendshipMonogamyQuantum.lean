...
This explicit finite-dimensional model supersedes the effective overlap-product
benchmark previously proposed in PR #72. It formalizes standard decoherence and
local indistinguishability; it makes no novelty claim.
...
/-- Both single-copy views are dephased. They share the laboratory factor;
this identity does not describe simultaneous access to disjoint physical regions. -/
...
/-- In this explicit two-copy state, considering each superobserver with one distinct
redundant record and the common laboratory does not give either observer access to branch coherence: the
