...
Report schema 2 separates conditional evidence from native predictions. Old schema-1
snapshots are rejected so applications cannot silently misread old evidence labels.
`Report.compare()` retains its interface. Rebuild and export to migrate snapshots.
The existing Python package has no added runtime dependencies.

Lean tests live in `Tests/`, Python tests in `python_tests/`. The Python runner rejects
zero discovered tests and the suite checks tracked path prefixes for case collisions.
All committed HTML is generated by `scripts/export.py`; CI checks for stale exports.
