...
def parse_searches(stdout: str) -> list[dict]:
    ...
    for line in stdout.splitlines():
        ...
        if status not in {'base-separates', 'no-candidate', 'found'}:
        ...
        if candidate is None:
            ...
            candidate_item = parse_comparisons(COMPARISON_PREFIX + json.dumps(
                {'declaration': item['declaration']+'.candidate', 'axioms': item['axioms'],
                 'report': candidate}))[0]
        if status == 'base-separates':
            if base['verdict'] != 'separation' or candidate_item is not None:
                raise ValueError('Base-separates search has inconsistent evidence')
        elif status == 'no-candidate':
            if base['verdict'] != 'agreement' or candidate is None or candidate['verdict'] != 'agreement':
                raise ValueError('No-candidate search has inconsistent evidence')
        else:
            if base['verdict'] != 'agreement' or candidate is None or candidate['verdict'] != 'separation':
